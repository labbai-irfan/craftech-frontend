...
const LeadsCRM = () => {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-4xl font-black text-navy mb-2">Lead Management</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
        {statuses.map(status => (
          <div key={status} className="bg-white p-4 rounded-lg border border-gray-200">
            <div className="text-2xl font-black text-navy">{leads[status]?.length || 0}</div>
            <div className="text-xs font-bold text-mid uppercase tracking-[1px]">{status}</div>
          </div>
        ))}
      </div>
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto">
              {leads[status]?.map(lead => (
                ...
                >
                  ...
                  {lead.projectType && (
                    <div className="text-xs bg-blue-50 text-blue-700 rounded px-2 py-1 mt-2 w-fit">
                      {lead.projectType}
                    </div>
                  )}
                </div>
        ...
        >
          ...
          >
            <div className="flex justify-between items-start mb-6">
              <h2 className="text-2xl font-black text-navy">{selectedLead.name}</h2>
              <button
                onClick={() => setSelectedLead(null)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm font-medium text-navy"
                  >
                    {statuses.map(s => (
                ...
                {editingId === selectedLead._id ? (
                  <div className="flex gap-2">
                    <textarea
                      ...
                      className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm font-medium resize-none"
                      rows="3"
                    />
                    <button
                  ...
                  <div
                    ...
                    className="bg-gray-50 p-3 rounded-lg text-sm text-navy min-h-[60px] cursor-pointer hover:bg-gray-100"
                  >
